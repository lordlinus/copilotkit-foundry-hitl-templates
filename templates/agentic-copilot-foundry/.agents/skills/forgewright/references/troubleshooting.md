...
| ⚠️ **CONFIRMED upstream bug, not this template's code:** after one approve, a LATER unrelated turn in the SAME conversation (e.g. a plain follow-up question) silently re-executes the SAME already-resolved gated tool again — its side effect applies more than once even though the user only clicked Approve once | Isolated by reproducing with **raw curl straight to the hosted agent's `/responses` endpoint** (`agent-framework-core==1.9.0`, `agent-framework-ag-ui==1.0.0rc5`, `agent-framework-foundry==1.8.2`), with zero involvement of CopilotKit, `bridge_app.py`, or `hosted_proxy.py` — the duplicate execution happens purely from `previous_response_id` chaining across an approval boundary, inside `ResponsesHostServer`/`FoundryChatClient`/`agent_framework_ag_ui` itself. Root cause: `_clean_resolved_approvals_from_snapshot` (`agent_framework_ag_ui/_agent_run.py`) only cleans up the *direct* `approval_mode` flow's stale approval content, not the `confirm_changes` flow this template uses — so the tool call never gets durably marked resolved, and gets reconsidered/re-executed on a later turn. This template's own `hosted_proxy.py` correctly identifies the later turn as normal and does NOT re-send an `mcp_approval_response` (verified via its own log) — the bug is entirely upstream of any of this template's code. **Tracked upstream:** [microsoft/agent-framework#6828](https://github.com/microsoft/agent-framework/issues/6828) (originally reported as a UI "chip reverts to running" symptom) and [microsoft/agent-framework#6851](https://github.com/microsoft/agent-framework/issues/6851) (filed separately for the more severe safety consequence — actual duplicate execution of the gated tool's real side effect, not just a display glitch; cross-linked to #6828). **Treat any multi-turn conversation following an approval as unsafe for real (non-demo) consequential actions until #6828/#6851 are fixed** — do not attempt a workaround in `backend/{bridge_app,hosted_proxy,hosted_client}.py`, since the defect is not in this template's code and a local patch would fight the framework's own internal state instead of fixing the cause. Re-check both issues on every `agent-framework-*` upgrade. |
