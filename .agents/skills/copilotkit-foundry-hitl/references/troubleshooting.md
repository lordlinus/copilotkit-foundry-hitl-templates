...
| `azd ai agent run` crashes immediately with `KeyError: 'FOUNDRY_PROJECT_ENDPOINT'` (or "Model is required…"), even run interactively in a real terminal | **`azd ai agent run` never prompts for or provisions a subscription/project on its own** — confirmed against Microsoft Learn's Foundry Hosted Agents quickstarts (which have the user `export`/`azd env set` `FOUNDRY_PROJECT_ENDPOINT` + `AZURE_AI_MODEL_DEPLOYMENT_NAME` BEFORE `azd ai agent run`) and live on azd 1.27.0 + `azure.ai.agents` 1.0.0-beta.5 (reproduced with a real PTY, not just a redirected stdin — no prompt ever appears; it silently runs with an empty env and the Python process crashes). The root LOCAL-DEV `azure.yaml` intentionally omits `FOUNDRY_PROJECT_ENDPOINT` (comment: "`azd ai agent run` injects it") — that comment does not match actual tool behavior. | `scripts/lib-agentrun.sh` auto-heals this: if the LOCAL azd env (`./.azure`) has no `FOUNDRY_PROJECT_ENDPOINT`, it reuses the one `make up` already provisioned in `hosted/.azure` (`azd env get-value` there → `azd env set` at the app root) before starting the agent — no extra manual step needed after `make up`. Don't provision a second, separate local-only Foundry project (e.g. via a bare `azd provision` at the app root) just to satisfy this — that project has no model `deployments:` block and 404s `DeploymentNotFound`; reusing `hosted/`'s already-deployed model is the fix. If `hosted/.azure` doesn't exist yet, run `make up` first, or set the value directly: `azd env set FOUNDRY_PROJECT_ENDPOINT https://<account>.services.ai.azure.com/api/projects/<project>`. `make doctor` checks this. |
