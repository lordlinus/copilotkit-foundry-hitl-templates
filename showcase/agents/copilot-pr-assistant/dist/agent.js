...
const CHANGED_FILES = [
    ...
    },
    {
        ...
        diff: "+ it('sets Secure + SameSite=Strict on the session cookie', () => {\n" +
...
let _prCounter = 0;
export function openPullRequest(p) {
    _prCounter += 1;
    return {
        number: _prCounter,
...
export function buildTools() {
    return [
        ...
        defineTool(PROPOSE_TOOL, {
            description: "Propose a pull request for the changes. This DRAFTS the PR and pauses for human approval; " +
            ...
            skipPermission: true,
            handler: async () => "Draft recorded. Awaiting human approval before opening. Do not call any more tools; wait for the decision.",
        }),
    ];
}
...
export const SYSTEM_MESSAGE = "You are a senior engineer that opens pull requests. Workflow:\n" +
...
export async function buildProvider() {
    ...
    if (!token)
        throw new Error("Failed to acquire an Azure AD token for the model provider.");
...
export async function getClient() {
    ...
    _client = new CopilotClient({ mode: "empty", baseDirectory, logLevel: process.env.SDK_LOG_LEVEL || "error" });
    await _client.start();
    return _client;
}
function required(name) {
    ...
    if (!v)
        throw new Error(`${name} is required (Azure BYOM). See .env.example.`);
    return v.replace(/\/+$/, "");
}
