...
const NOTE_TOOLS = [
  defineTool("save_note", {
    ...
    handler: async ({ text }: { text: string }) => ({ saved: text }),
  }),
];

const SYSTEM =
  "You manage a private notebook for ONE user. When asked to save a note, call save_note. " +
...
async function newClient(): Promise<CopilotClient> {
  const c = new CopilotClient({ mode: "empty", baseDirectory: process.env.COPILOT_HOME || `${process.cwd()}/.state`, logLevel: "error" });
  await c.start();
  return c;
}

async function session(client: CopilotClient, sessionId: string, resume: boolean) {
  const cfg = {
    sessionId,
    model: MODEL_NAME,
  ...
  return resume ? client.resumeSession(sessionId, cfg as any) : client.createSession(cfg as any);
}

async function ask(client: CopilotClient, sessionId: string, prompt: string, resume = false): Promise<string> {
