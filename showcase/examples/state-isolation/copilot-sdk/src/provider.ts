...
function startApimAdapter(gateway: string, pathPrefix: string, apiKey: string): Promise<string> {
  ...
  const server = createServer((cReq, cRes) => {
    ...
    for (const [k, v] of Object.entries(cReq.headers)) {
      if (typeof v === "string" && !["host", "api-key", "authorization", "content-length"].includes(k.toLowerCase())) headers[k] = v;
    }
...
export async function buildProvider() {
  ...
  if (process.env.APIM_GATEWAY && process.env.APIM_KEY) {
    if (!_apimBaseUrl) _apimBaseUrl = await startApimAdapter(process.env.APIM_GATEWAY, process.env.APIM_PATH_PREFIX || "", process.env.APIM_KEY);
    return { type: "azure" as const, baseUrl: _apimBaseUrl, apiKey: "apim", wireApi, azure: { apiVersion } };
  }
  const baseUrl = (process.env.AZURE_OPENAI_ENDPOINT || "").replace(/\/+$/, "");
  if (!baseUrl) throw new Error("Set APIM_GATEWAY+APIM_KEY or AZURE_OPENAI_ENDPOINT (+LLM_API_KEY).");
